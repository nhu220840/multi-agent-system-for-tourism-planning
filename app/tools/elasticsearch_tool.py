...
def _search_unified_catalog(terms: list[str], source_kind: str, top_k: int) -> list[dict]:
    ...
    for item in _load_unified_catalog():
        ...
        if category not in wanted_categories:
            continue
        city = str(item.get("city") or "")
        if city and city not in DEFAULT_CITIES and not any(token in _fold_text(city) for token in ("hoi an", "quang nam", "da nang", "danang")):
            continue
        blob = _fold_text(
            " ".join(
                [
...
def _merge_retrieval_rows(primary_rows: list[dict], vector_rows: list[dict]) -> list[dict]:
    ...
    def ingest(row: dict, *, from_vector: bool) -> None:
        ...
        current["elasticsearch_score"] = max(
            float(current.get("elasticsearch_score") or 0.0),
            float(incoming.get("elasticsearch_score") or 0.0),
        )
