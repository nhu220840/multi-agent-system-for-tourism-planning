...
def _candidate_context(label: str, pool: list[dict[str, Any]]) -> str:
    ...
    for item in pool[:4]:
        ...
        lines.extend(
            [
                f"- Tên: {item.get('name','')}",
                f"  - Giá tham khảo: {item.get('price_note','')}",
                f"  - Địa chỉ: {item.get('address','')}",
                f"  - Sao: {item.get('star_rating','')}",
                f"  - Gợi ý từ dữ liệu: {snippet}",
            ]
        )
    return "\n".join(lines)
...
def _llm_recommendations(
    *,
    ...
    prompt = f"""Bạn là trợ lý du lịch đang chọn khách sạn cho itinerary.

Yêu cầu:
- Chọn đúng 2 khách sạn KHÁC NHAU.
- Một khách sạn cho phân khúc "Bình dân".
- Một khách sạn cho phân khúc "Cao cấp".
- Phải dựa vào giá tham khảo và nội dung mô tả trong dữ liệu để giải thích.
- Không dùng lý do kiểu kỹ thuật như stay_db, retrieval, source.
- Ưu tiên khách sạn phù hợp với hành trình sau: {query}

Danh sách ứng viên:
{_candidate_context("Bình dân", budget_pool)}

{_candidate_context("Cao cấp", premium_pool)}

Chỉ được chọn tên nằm trong danh sách ứng viên ở trên.
...
"""
    ...
    kwargs = {
        ...
        "messages": [
            {"role": "system", "content": "Bạn là trợ lý tư vấn lưu trú du lịch."},
    ...
    for item in recs:
        ...
        if segment not in {"Bình dân", "Cao cấp"} or name not in allowed_set or name in used:
            continue
    ...
    if len(out) != 2:
        return []
    out.sort(key=lambda item: 0 if item["segment"] == "Bình dân" else 1)
    return out
...
def _fallback_recommendations(candidates: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    return [
        _fallback_item("Bình dân", budget),
        _fallback_item("Cao cấp", premium),
    ]
...
def _fallback_item(segment: str, item: dict[str, Any]) -> dict[str, Any]:
    ...
    if not reason:
        reason = "Co vi tri va thong tin luu tru kha phu hop de dua vao danh sach can nhac."
    return {
        "segment": segment,
